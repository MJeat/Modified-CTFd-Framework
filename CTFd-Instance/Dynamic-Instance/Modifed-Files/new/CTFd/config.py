...
class ServerConfig(object):
    ...
    # Check Environment Variable first, then config.ini, then default to True
    ...
    SESSION_COOKIE_SAMESITE: str = os.getenv(
        "SESSION_COOKIE_SAMESITE", empty_str_cast(config_ini["security"]["SESSION_COOKIE_SAMESITE"]) or "Lax"
    )

    # Set the session lifetime from the docker-compose.yaml for 24 hours and if that fails, it will expire in 7 days 
    PERMANENT_SESSION_LIFETIME: int = int(os.getenv("PERMANENT_SESSION_LIFETIME", 
        config_ini["security"].getint("PERMANENT_SESSION_LIFETIME") or 604800))
